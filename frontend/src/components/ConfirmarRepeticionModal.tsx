...
export default function ConfirmReplayModal({ seccionId: sectionId, titulo: title, aprobados: approvedCount, aprobado: isApproved, onClose, onConfirm }: Props) {
  ...
  const confirm = onConfirm ?? (() => navigate(`/seccion/${sectionId}`, { state: { iniciarRepeticion: true } }))
  return <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    <div role="dialog" aria-modal="true" aria-labelledby="titulo-repeticion" className="relative z-10 w-full max-w-lg rounded-lg bg-canvas p-6 shadow-card">
      ...
      <p className="mt-3 text-base leading-relaxed text-body">Tu nota actual (<strong>{approvedCount}/8</strong>) se mantiene mientras juegas. Solo se reemplaza cuando termines los 8. Si dejas la repetición a medias, conservas <strong>{approvedCount}/8</strong>.</p>
      {isApproved && <p className="mt-3 text-base leading-relaxed text-body">Cuenta tu <strong>última</strong> ronda completa, así que tu nota puede bajar si esta vez te va peor.</p>}
      <div className="mt-6 flex gap-3"><button type="button" onClick={confirm} className="min-h-11 flex-1 rounded-md bg-primary px-4 py-2 font-medium text-on-primary">Empezar la repetición</button><button type="button" onClick={onClose} className="min-h-11 rounded-md border border-hairline-strong px-4 py-2 text-body">Ahora no</button></div>
    </div>
  </div>
}
