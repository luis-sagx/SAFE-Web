...
const { reproducirResultadoMock, reproducirSenalMock, useSoundMock } =
  vi.hoisted(() => ({
...
describe("PanelVeredicto", () => {
  ...
  it("separa el repaso de señales con un fondo propio y conserva compacto el veredicto", () => {
    ...
    expect(within(signals).getByText("La señal cambia.").className).toContain(
      "text-signal-body",
    );
  });
  ...
  it("muestra el intento en cola como aviso temporal y no dentro del panel", async () => {
    ...
    expect(
      screen.getByText("Caíste").closest(".rounded-lg")?.contains(notice),
    ).toBe(false);
  });
  ...
  it("avisa sin encolar cuando el servidor rechaza definitivamente el intento", async () => {
    ...
    const notice = await screen.findByText(
      /no se pudo registrar este intento/i,
    );
    expect(notice.closest("[data-sonner-toast]")).not.toBeNull();
    expect(
      await screen.findByText(
        /fue rechazado y no volverá a enviarse automáticamente/i,
      ),
    ).toBeDefined();
  });
  ...
  it('reserva el alto con todas las señales reales (no con una "más larga" adivinada por longitud de texto)', () => {
        ...
        <VerdictPanel
          ...
          senales={[
            { id: "s1", texto: "Corta." },
            {
              id: "s2",
              texto:
                "La dirección era bancodellitoral.com.ec.seguridad-alertas.com, un dominio largo.",
            },
          ]}
          regla="Comprueba por otro canal."
          contenedorId="pantalla-escenario"
    ...
    expect(screen.getByText(/dominio largo/).className).toContain(
      "text-signal-body",
    );
  });
