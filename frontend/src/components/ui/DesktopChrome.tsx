import { Globe, Inbox, Send, ShieldAlert, Trash2, type LucideIcon } from 'lucide-react'
...
export interface PestanaNavegador {
  /** Id del nodo del grafo cuya pantalla muestra. */
  id: string
  titulo: string
  url: string
  segura: boolean
  /** `data-signal` de la barra de direcciones, si el repaso la señala. */
  senalUrl?: string
  /** A dónde lleva cerrarla. Sin esto la pestaña no se puede cerrar. */
  cierra?: string
}

/** Un sitio de la barra de marcadores. */
...
export function VentanaNavegador({
  ...
  /** Las pestañas abiertas, en orden. La primera de la lista es la primera que
   *  se abrió, igual que en un navegador. */
  pestanas: PestanaNavegador[]
  activa: string
  ...
  const actual = pestanas.find((pestana) => pestana.id === activa)
