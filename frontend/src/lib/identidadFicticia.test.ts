...
import { ACCOUNT_FAKE, IDENTITY_FAKE } from './identidadFicticia'
...
describe('identidad ficticia de los escenarios', () => {
  ...
  it('la cédula no puede pertenecerle a nadie', () => {
    expect(isEcuadorianId(IDENTITY_FAKE.cedula)).toBe(false)
  })

  it('el RUC se deriva de esa misma cédula', () => {
    expect(IDENTITY_FAKE.ruc).toBe(`${IDENTITY_FAKE.cedula}001`)
  })
  ...
  it('la contraseña se reconoce como del ejercicio', () => {
    expect(IDENTITY_FAKE.clave).toMatch(/practica/i)
    expect(IDENTITY_FAKE.clave.length).toBeGreaterThan(15)
  })

  it('la cuenta se lee con su banco', () => {
    expect(ACCOUNT_FAKE).toContain(IDENTITY_FAKE.cuenta)
    expect(ACCOUNT_FAKE).toContain(IDENTITY_FAKE.banco)
  })
})
