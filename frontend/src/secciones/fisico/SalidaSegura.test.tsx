...
vi.mock("../../context/AuthContext", async () =>
  (await import("../../test/escenario")).mockAuth(),
);
vi.mock("../../lib/api", async () =>
  (await import("../../test/escenario")).offlineApi(),
);

function zoomIn(scene: HTMLElement) {
  fireEvent.click(
    within(scene).getByRole("button", { name: "Acercarte a la pantalla" }),
  );
}
...
function secureEverything(scene: HTMLElement) {
  zoomIn(scene);
  fireEvent.click(
    within(scene).getByRole("button", { name: "Cerrar el navegador" }),
  );
  fireEvent.click(
    within(scene).getByRole("button", { name: "Volver al escritorio" }),
  );
  fireEvent.click(drawer(scene));
  fireEvent.click(
    within(scene).getByRole("button", { name: "Bloquear la sesión" }),
  );
}
...
describe("SalidaSegura", () => {
  it("abre con tres pestañas, tres documentos y la sesión sin bloquear", () => {
    ...
    zoomIn(scene);
    const tabs = within(scene).getAllByRole("button", {
      name: /^Cerrar la pestaña/,
    });
    ...
    expect(
      within(scene).getAllByRole("button", { name: /^Guardar / }),
    ).toHaveLength(3);
    // 3 pestañas + 3 documentos + sesión.
    expect(within(scene).getByRole("status").textContent).toBe(
      "Quedan 7 cosas a la vista",
    );
  });
  ...
  it("tocar una hoja la guarda en el cajón", () => {
    ...
    fireEvent.click(
      within(scene).getByRole("button", {
        name: "Guardar Contratos en el cajón",
      }),
    );

    expect(
      within(scene).queryByRole("button", {
        name: "Guardar Contratos en el cajón",
      }),
    ).toBeNull();
    expect(within(scene).getByRole("status").textContent).toBe(
      "Quedan 6 cosas a la vista",
    );
  ...
  it("arrastrar una hoja hasta el cajón la guarda; soltarla fuera la deja donde estaba", () => {
    const scene = start(<SafeExit />);
    vi.spyOn(drawer(scene), "getBoundingClientRect").mockReturnValue(
      new DOMRect(800, 100, 200, 300),
    );
    const sheet = () =>
      within(scene).queryByRole("button", {
        name: "Guardar Nóminas en el cajón",
      });
    ...
    const other = within(scene).getByRole("button", {
      name: "Guardar Contratos en el cajón",
    });
  ...
  it("tocar el cajón guarda todos los documentos", () => {
    ...
    expect(
      within(scene).queryAllByRole("button", { name: /^Guardar / }),
    ).toHaveLength(0);
  ...
  it("la ✕ de la ventana cierra todas las pestañas de una vez", () => {
    ...
    expect(
      within(scene).queryAllByRole("button", { name: /^Cerrar la pestaña/ }),
    ).toHaveLength(0);
    expect(
      within(scene).getByText("No queda ninguna pestaña abierta"),
    ).toBeDefined();
  });
  ...
  it("la sesión se bloquea con el botón a la vista de la barra de tareas", () => {
    ...
    expect(
      within(scene).queryByRole("button", { name: "Volver al escritorio" }),
    ).toBeNull();
    expect(
      within(scene).queryAllByRole("button", { name: /^Cerrar la pestaña/ }),
    ).toHaveLength(0);
  ...
  it("tocar la pantalla bloqueada la desbloquea y la abre", () => {
    ...
    lockFromTaskbar(scene);
    fireEvent.click(
      within(scene).getByRole("button", { name: "Desbloquear la sesión" }),
    );

    expect(
      within(scene).getAllByRole("button", { name: /^Cerrar la pestaña/ }),
    ).toHaveLength(3);
  });
  ...
  it("abre en la primera pestaña y muestra su URL", () => {
    ...
    expect(
      within(scene).getByText("intranet.andes.ec/rrhh/nominas"),
    ).toBeDefined();
  });
  ...
  it("cerrar la pestaña activa pasa a la de al lado, no a una ventana en blanco", () => {
    ...
    zoomIn(scene);
    fireEvent.click(
      within(scene).getByRole("button", { name: "Cerrar la pestaña Nóminas" }),
    );
  ...
  it("en el escritorio el monitor es una miniatura: para tocar pestañas hay que acercarse", () => {
    ...
    const paper = within(scene).getByRole("button", {
      name: "Guardar Contratos en el cajón",
    });
  ...
  it("el repaso de señales resalta la parte de la escena de la que habla", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Irme de la oficina" }));
    fireEvent.click(
      await screen.findByRole("button", { name: "Ver las señales" }),
    );

    await waitFor(() => {
      expect(
        scene
          .querySelector('[data-signal="pestanas"]')
          ?.classList.contains("senal-resaltada"),
      ).toBe(true);
    });
    // Y la escena vuelve a mostrar lo que ya se había cerrado: si no, no habría
    // nada que señalar.
    const monitor = scene.querySelector<HTMLElement>(
  ...
  it("en celular la miniatura del monitor no se reduce más allá de lo legible", () => {
    ...
    Object.defineProperty(monitor, "clientWidth", {
      value: 60,
      configurable: true,
    });
