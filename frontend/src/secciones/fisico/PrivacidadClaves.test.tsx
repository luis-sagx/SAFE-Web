...
function block(container: HTMLElement) {
  fireEvent.click(
    within(container).getByRole("button", { name: "Inicio y apagado" }),
  );
  fireEvent.click(
    within(container).getByRole("menuitem", { name: "Bloquear" }),
  );
}

function closeAll(container: HTMLElement) {
  for (const close of within(container).getAllByRole("button", {
    name: /^Cerrar /,
  })) {
...
describe("PrivacidadClaves", () => {
  it("abre con las tres aplicaciones y la sesión desbloqueada", () => {
    ...
    expect(
      within(container).getByRole("region", {
        name: "Bóveda Andes, Mis credenciales",
      }),
    ).toBeDefined();
    expect(
      within(container).getByRole("region", { name: "Explorador de archivos" }),
    ).toBeDefined();
    expect(
      within(container).getByRole("region", {
        name: "Navegador, Correo Andes",
      }),
    ).toBeDefined();
    expect(
      within(container).getAllByRole("button", { name: /^Cerrar / }),
    ).toHaveLength(3);
  });
  ...
  it("cerrar una ventana deja las otras dos en pie", () => {
    ...
    fireEvent.click(
      within(container).getByRole("button", {
        name: "Cerrar Explorador de archivos",
      }),
    );
  ...
  it("la barra de tareas lista lo abierto y salta a esa ventana", () => {
    ...
    expect(
      within(container).getByRole("button", { name: "Credenciales" }),
    ).toBeDefined();
    expect(
      within(container).getByRole("button", { name: "Archivos" }),
    ).toBeDefined();
    expect(
      within(container).getByRole("button", { name: "Navegador" }),
    ).toBeDefined();

    const vault = within(container).getByRole("region", {
      name: "Bóveda Andes, Mis credenciales",
    });
    const email = within(container).getByRole("region", {
      name: "Navegador, Correo Andes",
    });
    fireEvent.click(
      within(container).getByRole("button", { name: "Credenciales" }),
    );
    expect(Number(vault.style.zIndex)).toBeGreaterThan(
      Number(email.style.zIndex),
    );
    ...
    expect(
      within(container).queryByRole("button", { name: "Archivos" }),
    ).toBeNull();
  });
  ...
  it("pulsar una ventana la trae al frente", () => {
    ...
    expect(Number(email.style.zIndex)).toBeGreaterThan(
      Number(vault.style.zIndex),
    );
  ...
  it("el menú de encendido bloquea, y apagar no responde", () => {
    ...
    expect(
      within(container)
        .getByRole("menuitem", { name: "Apagar" })
    ...
    expect(
      within(container).queryAllByRole("button", { name: /^Cerrar / }),
    ).toHaveLength(0);
  ...
  it("cerrar todo y bloquear antes de girarse es la decisión segura", async () => {
    ...
    block(container);
    fireEvent.click(
      screen.getByRole("button", { name: "Girarme a atenderlo" }),
    );
  ...
  it("el repaso de señales reabre la aplicación de la que habla y la resalta", async () => {
    ...
    await waitFor(() => {
      expect(
        container
          .querySelector('[data-signal="credenciales"]')
    ...
    await waitFor(() => {
      expect(
        container
          .querySelector('[data-signal="correo"]')
          ?.classList.contains("senal-resaltada"),
      ).toBe(true);
    });
  });
