...
function encuestando(previas: { texto: string; mio?: boolean; senal?: string }[]): ScreenView {
  return {
    kind: 'call',
    quien: NUMERO,
    numero: 'Guayaquil, Ecuador',
...
const DIO_DATOS = encuestando([{ texto: `${NACIMIENTO}, y mi mamá es ${MADRE}.`, mio: true }])

const CEDIO = encuestando([
  { texto: '¿Y para una encuesta necesitan todo eso?', mio: true },
  {
    texto:
      'Es solo para validar que hablo con el titular, señora, es el protocolo. Si prefiere lo dejamos, aunque el beneficio de la comisión se registra hoy y mañana ya no le puedo ayudar.',
    senal: 'protocolo',
  },
...
const BUZON: ScreenView = {
  ...
  datos: [
    { etiqueta: 'Encuestas o campañas activas', valor: 'Ninguna', senal: 'sin-campana' },
...
export const STORY: Story<ScreenNode> = {
  ...
  e_datos: {
    ...
    outcome: `No hubo encuesta ni exoneración. Con tu fecha de nacimiento, el apellido de tu mamá, tu agencia y los últimos dígitos de tu tarjeta llamaron ellos al banco haciéndose pasar por ti: contestaron todas las preguntas de seguridad, pidieron una clave nueva de banca en línea y entraron a tu cuenta. Nunca te pidieron una contraseña porque no les hacía falta.`,
  },
  e_verifica: {
    kind: 'good',
    view: BUZON,
...
const RESUMEN = 'Una encuesta de satisfacción de tu banco te hace unas preguntas para "validarte".'

const CONTEXTO: Contexto = {
  antes: (
    <>
      Cliente del <strong>Banco del Litoral</strong> desde hace años. La semana pasada estuviste en
      la agencia y la fila estaba larguísima, así que una encuesta sobre la atención{' '}
      <strong>no te extraña</strong>.
    </>
  ),
  ahora: (
    <>
      <strong>A media tarde</strong> te llama un número de Guayaquil que no tienes guardado.
    </>
  ),
  detalle:
...
function EncuestaDatos() {
  return (
    <StoryEscenario
      ...
      pista={
        <p>
          Puedes no contestar, responder la encuesta, negarte, colgar en cualquier momento o dejar
          la llamada esperando y mirar en tu banca si esa encuesta existe. Fíjate menos en el tono y
          más en qué te están preguntando.
        </p>
      }
    />
  )
