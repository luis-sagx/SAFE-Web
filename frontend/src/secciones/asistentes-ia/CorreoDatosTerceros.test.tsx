...
function writeAndSend(container: HTMLElement, texto: string) {
  fireEvent.change(within(container).getByLabelText("Escribe tu mensaje"), {
    target: { value: texto },
  });
  fireEvent.click(
    within(container).getByRole("button", { name: "Enviar el mensaje" }),
  );
}
...
describe("CorreoDatosTerceros", () => {
  it("el chat alterna: contesta la IA, y recién entonces se ve el campo para escribir", () => {
    ...
    expect(
      within(container).getByText("Hola, ayúdame a redactar un correo."),
    ).toBeDefined();
    expect(
      within(container).getByText(/Cuéntame sobre qué asunto es el correo/),
    ).toBeDefined();
    expect(
      within(container).getByLabelText("Escribe tu mensaje"),
    ).toBeDefined();
  });
  ...
  it("el botón de enviar está deshabilitado mientras el campo está vacío", () => {
    const container = start(<ThirdPartyDataEmail />);
    const boton = within(container).getByRole("button", {
  ...
  it("escribir tres partes reales del nombre de la compañera es una fuga", async () => {
    ...
    expect(
      await screen.findByText("Datos personales compartidos con la IA"),
    ).toBeDefined();
  });
  ...
  it("dos partes del nombre de la compañera no causan pérdida", async () => {
    ...
    writeAndSend(
      container,
      "Es para pedir un cambio de horario a nombre de Andrea Cedeño.",
    );
    expect(
      await screen.findByText(
        "Correo redactado sin compartir datos reales de nadie",
      ),
    ).toBeDefined();
  });
  ...
  it("dos partes del docente son seguras, pero tres son una fuga", async () => {
    ...
    writeAndSend(
      seguro,
      "Es para pedirle al Ing. Marcelo Tapia un cambio de horario en Redes.",
    );
    expect(
      await screen.findByText(
        "Correo redactado sin compartir datos reales de nadie",
    ...
    writeAndSend(
      fuga,
      "Es para pedirle a Marcelo Alejandro Tapia un cambio de horario en Redes.",
    );
    expect(
      await screen.findAllByText("Datos personales compartidos con la IA"),
    ).toHaveLength(1);
  });
  ...
  it("el repaso de señales resalta el dato real dentro del mensaje que se envió", async () => {
    ...
    writeAndSend(
      container,
      "Es a nombre de Andrea Carolina Cedeño Mora, cédula 1799999990.",
    );
    expect(
      await screen.findByText("Datos personales compartidos con la IA"),
    ).toBeDefined();
  ...
  it("una cédula inventada con la misma forma no cuenta como fuga", async () => {
    ...
    writeAndSend(
      container,
      "A nombre de mi compañera, cédula 1234567890, materia de Redes.",
    );
    expect(
      await screen.findByText(
        "Correo redactado sin compartir datos reales de nadie",
  ...
  it("un mensaje sin asunto no aprueba: la IA repregunta y el chat sigue abierto", async () => {
    ...
    expect(
      await within(container).findByText(
        /Me falta información para redactarlo/,
      ),
    ).toBeDefined();
    expect(
      screen.queryByText(
    ...
    writeAndSend(
      container,
      "Es para pedirle al profesor un cambio de horario en Redes.",
    );
    expect(
      await screen.findByText(
        "Correo redactado sin compartir datos reales de nadie",
