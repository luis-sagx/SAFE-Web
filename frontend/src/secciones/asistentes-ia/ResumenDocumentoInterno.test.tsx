...
describe('ResumenDocumentoInterno', () => {
  ...
  it('las burbujas no delatan qué comparte cada camino', () => {
    ...
    expect(within(container).getByRole('button', { name: 'Le paso el informe y le pido que lo resuma.' })).toBeDefined()
    expect(within(container).queryByText(/\$340\.000/)).toBeNull()
  })

  it('pasar el informe filtra información confidencial, y el mensaje enviado la muestra', () => {
    const container = start(<InternalDocumentSummary />)
    fireEvent.click(within(container).getByRole('button', { name: 'Le paso el informe y le pido que lo resuma.' }))
    expect(screen.getByText('Información confidencial de la empresa compartida con la IA')).toBeDefined()
    expect(container.querySelector('[data-signal="dato-perdidas"]')?.textContent).toBe('$340.000')
    for (const signal of ['dato-recorte', 'dato-sin-avisar']) {
  ...
  it('pedir solo el párrafo modelo, sin las cifras, es el acierto', () => {
    const container = start(<InternalDocumentSummary />)
    fireEvent.click(
      within(container).getByRole('button', { name: 'Le pido un párrafo modelo y yo pongo las cifras aparte.' }),
    )
  ...
  it('no usar la IA evita el riesgo pero queda como respuesta incompleta', () => {
    const container = start(<InternalDocumentSummary />)
    fireEvent.click(within(container).getByRole('button', { name: 'Mejor lo resumo yo, gracias.' }))
