...
describe("BonoEstado", () => {
  it("se decide tocando el propio teléfono, sin lista de opciones", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Ver contexto y mis datos" }),
    ).toBeDefined();
    ...
    expect(
      within(phone).getByText("Acreditación del bono de $180"),
    ).toBeDefined();
    expect(
      within(phone).getByText("bono-social-ec.online/registro"),
    ).toBeDefined();

    fireEvent.click(
      within(phone).getByRole("button", { name: "Acreditar mi bono" }),
    );
  ...
  it("salir de la página falsa no entrega datos", () => {
    ...
    fireEvent.click(
      within(phone).getByRole("button", { name: "Volver atrás" }),
    );
    expect(
      screen.getByText("No caíste · el formulario te delató"),
    ).toBeDefined();
  });
  ...
  it("volver al navegador desde el hilo no deja la pantalla en Mensajes", () => {
    const container = start();
    const phone = container.querySelector("#pantalla-escenario") as HTMLElement;
    const app = (name: RegExp) =>
      within(phone).getByRole("button", { name: name });
  ...
  it("cerrar una app abierta sobre el hilo descubre ese mismo hilo", () => {
    ...
    fireEvent.click(
      within(phone).getByRole("button", { name: "Volver al hilo de mensajes" }),
    );
  ...
  it("abrir el navegador no comprueba nada, pero entrar al portal oficial sí", () => {
    ...
    fireEvent.click(
      within(phone).getByRole("button", { name: /inclusion\.gob\.ec/ }),
    );
    ...
    expect(
      screen.getByText("No caíste · buscaste la fuente oficial"),
    ).toBeDefined();
  });
