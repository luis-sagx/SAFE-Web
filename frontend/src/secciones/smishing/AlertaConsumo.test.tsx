...
import ConsumptionAlert from './AlertaConsumo'
...
describe('AlertaConsumo', () => {
  it('permite escoger y enviar directamente el mensaje predeterminado', () => {
    ...
    expect(within(phone).getByText(/4539 0011 8842 4417/)).toBeDefined()
  ...
  it.skip('pedir el bloqueo con el número de la tarjeta es el fallo', () => {
    ...
    fireEvent.click(within(phone).getByRole('button', { name: /Bloquéenme la tarjeta/ }))

    // Ya no es una burbuja para elegir: es un mensaje mandado, con el número.
    expect(within(phone).queryByRole('button', { name: /Bloquéenme la tarjeta/ })).toBeNull()
    expect(within(phone).getByText(/4539 0011 8842 4417/)).toBeDefined()
    expect(screen.getByText('Aviso legítimo, reacción peligrosa')).toBeDefined()
  })
  ...
  it.skip('pedir que te llamen no entrega nada, pero te deja esperando una llamada', () => {
    ...
    fireEvent.click(within(phone).getByRole('button', { name: /Llámenme por favor/ }))
  ...
  it('las apps que no deciden se abren igual y se vuelve con la flecha', () => {
    ...
    expect(within(phone).getByText(/SUPERMERCADO LA UNIÓN/)).toBeDefined()
  })
  ...
  it('releer el hilo desde la app del banco no termina la corrida', () => {
    ...
    expect(
      within(phone).queryByRole('button', { name: 'Volver a la lista de mensajes' }),
    ).toBeNull()
    expect(screen.getByText('¿Qué haces?')).toBeDefined()
  })
  ...
  it('verificar es usar la app, no abrirla', () => {
    ...
    expect(within(phone).getByText('Tarjeta *4417')).toBeDefined()
    expect(within(phone).queryByText('bancolitoral.ec')).toBeNull()
    expect(screen.getByText('¿Qué haces?')).toBeDefined()

    fireEvent.click(within(phone).getByRole('button', { name: /Movimientos/ }))

    // El consumo se ve en pantalla, y verlo ya es la comprobación: no queda
    // nada abierto después, así que la corrida termina aquí.
    expect(within(phone).getByText('Últimos consumos')).toBeDefined()
    expect(within(phone).getByText('$42,90')).toBeDefined()
