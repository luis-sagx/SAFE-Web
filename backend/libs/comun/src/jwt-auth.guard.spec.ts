...
describe('guards de rol', () => {
  it('permite a ADMIN y rechaza los demás roles para administración', () => {
    ...
    expect(() => guard.canActivate(contextWithRole('TRAINER'))).toThrow(
      ForbiddenException,
    );
    expect(() => guard.canActivate(contextWithRole('PARTICIPANT'))).toThrow(
      ForbiddenException,
    );
  });
  ...
  it('permite solo a PARTICIPANT registrar progreso y certificado', () => {
    ...
    expect(() => guard.canActivate(contextWithRole('ADMIN'))).toThrow(
      ForbiddenException,
    );
  });
});
