...
function runFixture(overrides: Record<string, unknown> = {}) {
  return {
    ...
    participant: { seq: 7, cohort: 'comerciantes' },
    ...overrides,
  };
}
...
describe('RunsService.exportCsv', () => {
  it('emite la cabecera y una fila por corrida', async () => {
    ...
    expect(row).toBe(
      'P007,comerciantes,smishing/cambio-numero,1,CORRECTO,100,e_verifica,42000,2026-08-01T10:00:00.000Z,2026-08-01T10:00:42.000Z',
    );
  });

  // Esta es la garantía de privacidad del estudio: aunque el modelo tenga
  // nombre, correo y teléfono, la exportación no puede dejarlos salir.
  it('nunca exporta datos personales aunque vengan en la consulta', async () => {
  ...
  it('escapa comas y comillas para no correr las columnas', async () => {
    const csv = await serviceWith([
      runFixture({
        participant: { seq: 8, cohort: 'adultos, mayores' },
        endingId: 'dijo "no"',
      }),
    ]).exportCsv();
  ...
  it('deja la cohorte vacía cuando el participante no tiene grupo', async () => {
    const csv = await serviceWith([
      runFixture({ participant: { seq: 9, cohort: null } }),
    ]).exportCsv();
